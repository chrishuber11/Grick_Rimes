...
#Set file path to scripts directory
...
class MyGame(arcade.Window):
    ...
    def __init__(self, width, height, title):
        ...
        audio = arcade.load_sound('sound/Jeremy Blake - Powerup!.mp3')
        arcade.play_sound(audio,.25,True)
        ...
        if not merge_sprites:
            for row in range(dg.height):
                for column in range(dg.width):
                    ...
                    if value == '#':
                        wall = arcade.Sprite('assets/wall_new.png', wall_scale)
            ...
            for row in range(dg.height):
                ...
                while column < dg.width:
                    ...
                    wall = arcade.Sprite('assets/wall_new.png', wall_scale, repeat_count_x = column_count)
                    wall.center_x = column_mid * wall_size + wall_size / 2
                    wall.center_y = row * wall_size + wall_size / 2
                    wall.width = wall_size * column_count
                    self.wall_list.append(wall)
        self.player_sprite = arcade.Sprite('assets/GrickRimes.png', player_scale)
    ...
    def on_draw(self):
        ...
        arcade.draw_text(player_health_display, self.view_left + 20, screen_height -120 + self.view_bottom, arcade.color.BLACK, 16)

        output = f'Zombies: {self.zombie_count}'
        arcade.draw_text(output, self.view_left + 20, screen_height - 60 + self.view_bottom, arcade.color.BLACK, 16)
    ...
    def spawn_zombie(self):
        ...
        if chance > 7:
            self.zombie_sprite = arcade.Sprite('assets/me_real.png', player_scale)
        else:
            self.zombie_sprite = arcade.Sprite('assets/zombie_real.png', player_scale)
    ...
    def on_update(self, delta_time):
        ...
        if self.zombie_count < 5:
            ...
            if zombie_spawn > 100 and zombie_spawn < 120:
                self.spawn_zombie()
                self.zombie_count += 1
